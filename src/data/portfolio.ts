  ...
  experiences: [
    {
      ...
      description: [
        "Flutter 클라이언트 및 Node.js 백엔드 환경에서 크로스플랫폼 운동 관리 서비스 아키텍처 설계 및 제품 개발",
        "Riverpod 기반 MVVM 클라이언트 구조 수립, Socket.io 실시간 채팅 및 백그라운드 토큰 자동 갱신 소켓 재연결 파이프라인 구현",
        "Google Calendar API 연동 예약 동기화 개발 및 PT 패키지 정산 및 예약 비즈니스 로직 구현",
    ...
    },
    {
      ...
      description: [
        ...
        ".NET 6 마이그레이션을 통해 런타임 및 라이브러리 환경을 현대화하고 유지보수성을 개선",
      ],
    },
    {
      company: "전능아이티 (개발2팀)",
      ...
      description: [
        ...
        "프로그램 성능 개선: 통계 화면의 데이터 로딩 쿼리 최적화를 통해 조회 속도 향상. 요구사항 추가로 인해 새로고침 시 함께 갱신되는 데이터가 늘어나면서 느려진 새로고침 로직을 분석하여 불필요한 재조회 제거등으로 개선",
      ],
    },
    {
      company: "전능아이티 (개발2팀)",
      ...
      description: [
        ...
        "레이어 간 의존성이 명확하지 않아 발생하던 높은 결합도와 중복 로직을 개선하기 위해 계층형 아키텍처를 도입",
        "휴진 기능 추가 및 인센티브 기능 추가",
        "dottrace를 사용하여 느린 부분을 확인 후 속도 개선",
        "신규 수납 기능의 대한 통계 개발",
        "이미지 관리 프로그램 초기 실행 속도 개선: ORM DB 연결 지연(1~2초), UI 렌더링 및 이미지 로드 등이 겹쳐 3~5초 걸리던 초기 실행을 1초 미만으로 단축. MMF(Memory Mapped File) 기반 Shared Memory를 도입해 프로그램을 실행 상태로 유지하고 데이터만 갱신하는 방식으로 전환하여, 자주 열고 닫던 프로그램의 반복적인 초기화 비용을 제거함으로써 해결",
        "고객 관리 프로그램 티켓 기능 추가",
        "속도 개선을 위해 레거시 코드 리팩터링",
      ],
    },
  ] as Experience[],
  projects: [
    {
      ...
      contribution: "클라이언트(Flutter) 설계 및 개발 주도, 백엔드(Node.js/API) 개발 기여",
      longDescription:
    ...
    },
    {
      ...
      techStack: [".NET 6", "C#", "EF6", "Migration"],
      period: "2024.06 - 2024.08",
      role: "소프트웨어 개발자",
      contribution: "기여도 100% (이관 작업 전담)",
      longDescription:
        "노후화된 기존 .NET Framework 버전을 .NET 6 환경으로 마이그레이션한 프로젝트입니다.",
      keyAchievements: [
        "구버전 외부 종속 DLL 패키지 호환성 분석 및 대체 라이브러리 교체를 주도하여 라이브러리 마이그레이션 완수",
        "System.Data.SqlClient를 최신 Microsoft.Data.SqlClient로 변경하고, 프레임워크 변경에 따른 DB 연동 이슈 해결",
        ".NET 6 마이그레이션 과정에서 발생한 LINQ 쿼리 동작 방식 차이 및 ORM 호환성 이슈를 조사하여 쿼리 리팩터링 완료",
      ],
      challenges:
        ".NET Framework에서 .NET 6로 마이그레이션하는 과정에서 DB Provider(System.Data.SqlClient → Microsoft.Data.SqlClient) 변경 및 프레임워크 런타임 호환성 차이로 인해 일부 LINQ 쿼리가 의도와 다르게 실행되는 이슈가 있었습니다. 이상 동작을 보이는 LINQ 구문을 전면 분석하여 .NET 6 환경에 맞게 쿼리를 리팩터링하고 예외 문제를 해결했습니다."
    },
    {
      ...
      longDescription:
        "기존 Windows Forms 기반으로 구축되었던 프로그램을 WPF 아키텍처로 변경한 프로젝트입니다.",
      keyAchievements: [
        "MVVM 패턴을 적용하여 UI와 비즈니스 로직 간의 의존성을 분리하고, 가독성 및 유지보수성 향상",
      ],
    },
    {
      ...
      keyAchievements: [
        ...
        "EF 사용 시 빈번하던 N+1 쿼리 문제를 쿼리 로그 분석을 통해 파악하고 Include 및 Select 프로젝션 적용으로 로딩 속도 30% 단축",
      ]
    },
    {
      title: "티켓 기능 개발",
      ...
      keyAchievements: [
        ...
        "티켓 관리 체계 구축을 위해 RDB 테이블 구조 설계 및 관계 정립(고객, 티켓, Log, 상태 등)",
        "C# 환경에서 Entity Framework를 활용하여 티켓 생성, 삭제, 수정, 검색 비즈니스 로직 개발",
